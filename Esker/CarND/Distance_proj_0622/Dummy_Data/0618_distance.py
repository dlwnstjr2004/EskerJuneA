...
#거리재려면 일단 settings.py 가 필요하고, projection.p 가 필요. (보이는 화면을 상향뷰로 바꾸고, 그랬을때 거리를 측정하기 때문!
...
class Car:
    ...
    def calculate_position(self, bbox):
        if (self.has_position):
            ...
            return np.array((self.warped_size[1]-dst[1])/self.pix_per_meter[1]) #이건 현재의 위치를 보여주는 것 뿐..
        else:
            return np.array([0])
    ...
    def draw(self, img, color=(255, 0, 0), thickness=2):
        if self.display:
            ...
            if self.has_position:
                ...
                cv2.putText(img, "RVel: {:6.2f}km/h".format(self.position.speed()[0]*self.fps*3.6), (int(window[0]), int(window[3]+20)),
                            cv2.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=2, color=(0, 0, 0))
                print(self.position.output()[0]
                #그냥 가독성을 위해 흰색폰트바탕에 검은색폰트글씨를 씀
...
class CarFinder:
    ...
    def find_cars(self, img, threshold = 1, reset = False):
        ...
        for bbox in bboxes:
            self.cars.append(Car(bbox, self.first, self.warped_size, self.transformation_matrix, self.pix_per_meter)) #warped_size는 setting에서 줬던 크기
...
if __name__ == '__main__':
    ...
    while(cap.isOpened()):
        ...
        #res_img = process_image(frame, cf, lf, cam_matrix, dist_coeffs, True)
        res_img = process_image(frame, cf, cam_matrix, dist_coeffs, True)
        ...
        #print(str(round((1000/(1000*exec_time)),1)))
