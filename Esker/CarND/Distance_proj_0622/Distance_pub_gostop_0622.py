...
if __name__ == '__main__':
    ...
    ##the publish topic is DISTANCE and its type is String (this is not a str)
    publisher = rospy.Publisher('/DISTANCE', String, queue_size = 10)
    rate = rospy.Rate(20)
    
    ##init bbox (need to replace car find axid)
    ...
    while(cap.isOpened()and not rospy.is_shutdown()):
        ...
        if (counter < 10):
            bbox[3] = bbox[3]-10 #make distance change
            counter = counter + 1
        else:
            bbox[3] = bbox[3]+10
            counter = counter + 1
        print(float(calculate_position(bbox))) #distance calculate
        
        #show current image
        newimg = Cutting_image(frame,300,300,500,500)
        
        #send a msg in topic : DISTANCE on data.data
        sender = float(calculate_position(bbox))
        publisher.publish(str(sender)) #type what you what to publish
        
        cv2.imshow('result',newimg)
        time.sleep(1) #for more execution time
