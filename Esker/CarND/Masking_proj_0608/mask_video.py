...
from mask_lane_finder import LaneFinder
...
if __name__ == '__main__':
    ...
    def process_image(img, mask_lane_finder, cam_matrix, dist_coeffs, reset = False):
        img = cv2.undistort(img, cam_matrix, dist_coeffs)
        mask_lane_finder.find_lane(img, distorted=False, reset=reset)
        return mask_lane_finder.draw_lane_weighted(img)#mask_lane_finder.draw_lane_masked(img)
