...
class LaneFinder:
    ...
    def draw_lane_masked(self, img):
        ...
        lanes = np.zeros((1280, 720, 3), dtype=np.uint8) #안된다ㅋㅋ
        cv2.bitwise_and(img,both_lines,lanes)
        return lanes
    ...
    def draw_lane_weighted(self, img, thickness=5, alpha=0.8, beta=1, gamma=0):
        ...
        if self.found:
            cv2.fillPoly(lanes, [both_lines.astype(np.int32)], (255, 255, 255)) # (0,255,0)
            cv2.polylines(lanes, [left_line.astype(np.int32)], False, (255, 255, 255),thickness=thickness ) #(255,0,0)
            cv2.polylines(lanes, [right_line.astype(np.int32)],False,  (255, 255, 255), thickness=thickness) #(0,0,255)
            cv2.fillPoly(lanes, [both_lines.astype(np.int32)], (255, 255, 255)) #0,255,0
        ...
        print(img.dtype)
        print(lanes.shape)
        img1 = np.zeros((720,1280,3),dtype=np.uint8)+255 #white background if +160 here => light gray background
        
        img2 = np.zeros((720,1280,3),dtype=np.uint8)#img2 = img.copy()
        cv2.circle(img2,(640,360), 60,(255,255,255),-1) #make a mask
        
        img3 = np.zeros((720,1280,3),dtype=np.uint8)
        cv2.bitwise_and(img1, img2, img3)

        cv2.imshow('img',img3)
        '''
        
        '''
        newimg1 = np.zeros((720,1280,3),dtype=np.uint8)+255

        np.where(lanes_unwarped != (0,0,0), (255,255,255), lanes_unwarped)
        result_result = np.where(lanes_unwarped != (0,0,0))
        print("my found : ", result_result)

        #cv2.imshow('lanes_unwarped',lanes_unwarped)
        '''

        masked_img = np.zeros((720,1280,3),dtype=np.uint8)
        cv2.bitwise_and(img, lanes_unwarped, masked_img)

        #cv2.imshow('newimg',newimg3)

        #print(lanes_unwarped.shape)
        
        '''
        for x in lanes_unwarped.shape[0]:
            for y in lanes_unwarped.shape[1]:
                for c in lanes_unwarped.shape[2]:
                    if lanes_unwarped[x,y,c] != 0:
                        lanes_unwarped[x,y,c] = 255
        '''
        return masked_img#cv2.addWeighted(img, alpha, lanes_unwarped, beta, gamma)
