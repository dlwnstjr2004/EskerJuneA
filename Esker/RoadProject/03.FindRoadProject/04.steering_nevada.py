...
cap = cv2.VideoCapture('nevada.mp4') # you can change this to challenge
...
def detect_lanes_img(img):
    ...
    # Set ROI
    vertices = np.array([[(150,height-10),(width/2-40, height/2+200), (width/2+40, height/2+200), (width-150,height-10)]], dtype=np.int32)	
    ...
    vertices2 = np.array([[(150,height-10),(width/2-40, height/2+200), (width/2+40, height/2+200), (width-150,height-10)]], dtype=np.int32)
...
#### my own code ####
    location_left = (50,50)
    location_right = (50,100)
    location_left_slope = (50,150)
    location_right_slope = (50,200)
    location_steering = (50,250)
    font = cv2.FONT_ITALIC
    fontScale = 1
    
    
    #cv2.putText(img,'left:'+str(left_fit_line),location_left,font,fontScale, (255,255,0), 2)
    #cv2.putText(img,'right:'+str(right_fit_line),location_right,font,fontScale, (255,255,0), 2)
    
    cv2.putText(img,'less than 90: steering left, more than 90: steering right',location_right,font,fontScale, (0,0,255), 2)

    left_slope_degree = (np.arctan2(left_fit_line[1] - left_fit_line[3], left_fit_line[0] - left_fit_line[2]) * 180) / np.pi
    right_slope_degree = (np.arctan2(right_fit_line[1] - right_fit_line[3], right_fit_line[0] - right_fit_line[2]) * 180) / np.pi

    cv2.putText(img,'left slope:'+str(left_slope_degree),location_left_slope,font,fontScale, (255,255,0), 2)
    cv2.putText(img,'right slope:'+str(right_slope_degree),location_right_slope,font,fontScale, (255,255,0), 2)
    
    steering_slope = (np.arctan2(right_fit_line[1] - right_fit_line[3], (right_fit_line[0]+left_fit_line[0])/2 - (right_fit_line[2]+left_fit_line[2])/2) * 180) / np.pi
    cv2.putText(img,'steering:'+str(steering_slope),location_steering,font,fontScale, (255,255,0), 2)
