...
def test_simple(args):
    ...
    #summary(encoder, (3,640, 192))

    x = torch.ones((1,3,640,192)).cuda()
    encoder_trt = torch2trt(encoder, [x])

    torch.save(encoder_trt.state_dict(), 'encoder_trt.pth')
    ...
    #print("Model's state_dict:")
    #for param_tensor in depth_decoder.state_dict():
    #    print(param_tensor, "\t", depth_decoder.state_dict()[param_tensor].size())

    y = torch.ones((1,512,20,6)).cuda()
    depth_decoder_trt = torch2trt(depth_decoder, [y])

    vid = cv2.VideoCapture(args.video_path)
    
    # FINDING INPUT IMAGES
    if(args.video_path == '0' or args.video_path == '1'):
        vid = open_cam_usb(args.video_path, 1920, 1080)
        print("camera open")
    ...
    with torch.no_grad():
        while(True):
            ...
            features = encoder_trt(input_image)
            outputs = depth_decoder_trt(features)
