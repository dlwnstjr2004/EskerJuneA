...
def test_simple(args):
    ...
    with torch.no_grad():
        while(True):
            ...
            #1st method -> too much meaningless image data processing
            '''
            mask_img = np.zeros((frame.shape[0], frame.shape[1], frame.shape[2]), np.uint8)
            cv2.rectangle(mask_img, ( (int)(width/2-350), height-60 ), ( (int)(width/2+350), height ), (255,255,255), thickness = 60)
            dst_img = np.zeros((frame.shape[0], frame.shape[1], frame.shape[2]), np.uint8) 
            cv2.bitwise_and(frame, mask_img, dst_img)
            '''
            #2nd method -> only processing what we need to find lines =>  progress time -59ms than 1st method
            half_width = (int)(width/2)
            dst_img = frame[height-60:height, half_width-350:half_width+350]
