...
def motor_control(width, center):
    ...
    #serialreturn = serialreturn+'a'
    if((pixelsize >= 400) or (pixelsize < 67)):
        serialreturn = serialreturn+'8'
    else:
        speedlimit = 500/pixelsize
        '''
        if speedlimit < 10 :
            serialreturn = serialreturn+'00'+str(int(speedlimit))
        elif speedlimit < 100 :
            serialreturn = serialreturn+'0'+str(int(speedlimit))
        else:
            serialreturn = serialreturn+str(int(speedlimit))
        '''
        serialreturn = serialreturn+str(int(speedlimit))
    serialreturn = serialreturn+','
    charcenter = str(int(center))
    '''
    if center < 10 :
        serialreturn = serialreturn+'00'+str(int(center))
    elif center < 100 :
        serialreturn = serialreturn+'0'+str(int(center))
    else:
        serialreturn = serialreturn+str(int(center))
    '''
    serialreturn = serialreturn+str(int(center))
    serialreturn = serialreturn+'e' #this is the end of string

    print('speed: '+str(int(speedlimit)))
    print('center: '+str(int(center)))

    ser.write(serialreturn.encode())
    return width, center
...
def steering_func(frame):
    ...
    dst_img = frame[height-60:height, 0:width]
...
def test_simple(args):
    ...
    with torch.no_grad():
        while(True):
            ...
            imcv_edges = edge_detect_func(imcv, 30, 130)
